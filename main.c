...
int main(int argc, char *argv[])
{
	...
	fprintf(stdout, "%s: %s %s - Matt Layher, March 2012\n", SERVER_NAME, INFO_MSG, SERVER_NAME);
...
void *tcp_listen()
{
   	...
   	while(1)
	{
		...
		else
		{
			...
			// On client connection, add work to the threadpool.
			thpool_add_work(threadpool, &echo, (void*)&inc_fd);
		}
	}
}
